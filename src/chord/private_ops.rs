...
use super::{
    message::{Packet, PacketType, PrivateMessage, PublicMessage},
    state::ChordState,
    ProcessorId,
};

impl<A: ChordAddress, I: ChordId, ADAPTOR: ChordAdaptor<A, I>> Chord<A, I, ADAPTOR> {
    pub(crate) async fn process_private(
        &mut self,
        channel_id: ProcessorId<I>,
        operation: PrivateMessage<A, I>,
    ) -> ChordResult {
        match operation {
    ...
    async fn stabilize(&mut self, channel_id: ProcessorId<I>) -> ChordResult {
    ...
    async fn fix_fingers(&mut self, channel_id: ProcessorId<I>) -> ChordResult {
        // make sure that message came from internal channel
        if let ProcessorId::Internal = channel_id {
            ...
            let packet_type = PacketType::Dialback {
    ...
    async fn check_predecessor(&mut self, channel_id: ProcessorId<I>) {
        if let ProcessorId::Internal = channel_id {
            ...
            if let Some(predecessor) = self.predecessor.clone() {
                // this node thinks it has a predecessor,
                if !self.members.contains_key(&predecessor) {
                    // but cannot find its connection as a member,
    ...
    async fn cleanup(&mut self, channel_id: ProcessorId<I>) {
        ...
        if let ProcessorId::Internal = channel_id {
            // cleanup members
            self.members.retain(|_, (_, ch)| !ch.is_closed());

            // cleanup associates
            self.associates.retain(|_, ch| !ch.is_closed());
        }
    }

    async fn save_state(&mut self, channel_id: ProcessorId<I>) -> ChordResult {
        // make sure that message came from internal channel
        if let ProcessorId::Internal = channel_id {
            if let Some(path) = &self.file_path {
                ...
                state.save(path).await.problem(ErrorKind::SaveFailure)?;
            }
        }
        Ok(())
    }
