...
use crate::{
    chord::{
        message::{Message, PrivateMessage, PublicMessage},
        ProcessorId,
    },
    chord_id::ChordId,
...
impl<A: ChordAddress, I: ChordId> AssociateChannel<A, I> {
    ...
    #[instrument(skip_all)]
    pub async fn duplicate(&self) -> ChordResult<Self> {
        ...
        self.to
            .send((
                ProcessorId::Associate(new_id),
                Message::Private(PrivateMessage::RegisterAssociate { conn: new_to }),
            ))
            .await
    ...
    pub async fn send_op(&self, msg: AssociateRequest<A, I>) -> ChordResult {
        self.to
            .send((ProcessorId::Associate(self.associate_id), msg.into()))
            .await
    ...
    pub async fn recv_op(&mut self, timeout: Option<u32>) -> ChordResult<AssociateResponse<A, I>> {
        loop {
            let msg = if let Some(limit) = timeout {
                let limit = tokio::time::Duration::from_millis(limit.into());
                time::timeout(limit, self.from.recv()).await?
            } else {
            ...
            match msg {
                ...
                None => return Err(ErrorKind::AssociateClosed)?,
            }
        }
    }
    ...
    pub async fn successor_of(&mut self, id: I) -> ChordResult<Option<(I, A)>> {
        self.send_op(AssociateRequest::GetSuccessorOf { id })
            .await?;
        loop {
            let response = self.recv_op(Some(10)).await?;
            match response {
                AssociateResponse::SuccessorOf { id, addr } => {
                    return Ok(Some((id, addr)));
                }
                _ => {
                    return Err(ErrorKind::MismatchedResponse)?;
                }
            }
        }
    }
    ...
    ///
    /// If other responses arrive, they are discarded.
    pub async fn advert_of(&mut self, id: I) -> ChordResult<Option<Vec<u8>>> {
        self.send_op(AssociateRequest::GetAdvertOf { id: id.clone() })
            .await?;
        loop {
            let response = self.recv_op(Some(10)).await?;
            match response {
                AssociateResponse::AdvertOf {
                    id: advert_id,
                    data,
                } => {
                    if advert_id != id {
                ...
                _ => return Err(ErrorKind::MismatchedResponse)?,
            }
        }
    }
}
...
impl<A: ChordAddress, I: ChordId> From<AssociateRequest<A, I>> for PublicMessage<A, I> {
    fn from(req: AssociateRequest<A, I>) -> Self {
        match req {
            ...
            AssociateRequest::Debug => PublicMessage::Debug {
                msg: "".to_string(),
            },
            AssociateRequest::Marker { .. } => {
                panic!("This is just a marker to quiet the compiler")
            }
        }
    }
}
    ...
    fn from(msg: PublicMessage<A, I>) -> Self {
        match msg {
            ...
            PublicMessage::Predecessor { pred } => {
                Some(AssociateResponse::Predecessor { id: pred })
            }
            ...
            PublicMessage::PeerAddresses { addrs } => {
