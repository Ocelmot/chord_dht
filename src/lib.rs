...
pub struct Chord<A, I: ChordId, ADAPTOR: ChordAdaptor<A, I>>{
    processor_input: Sender<(ProcessorId<I>, ChordOperation<I>)>,
    processor_output: Receiver<ChordOperationResult<I>>,

    adaptor: Arc<Mutex<ADAPTOR>>,
    processor_handle: JoinHandle<()>,
    listener_handle: JoinHandle<()>,
    marker: PhantomData<A>,
}

impl<A, I: ChordId, ADAPTOR: ChordAdaptor<A, I>> Chord<A, I, ADAPTOR>{


    pub async fn new(self_id: I, listen_addr: A, join_addr: Option<A>) -> io::Result<Chord<A, I, ADAPTOR>>{
        
        let (processor_output_tx, processor_output_rx) = channel::<ChordOperationResult<I>>(50);

        // Create and start the message processor
        let processor = ChordProcessor::new(self_id);
        ...
        let listener_handle = adaptor.listen_handler(listen_addr, processor_input.clone());
        let adaptor = Arc::new(Mutex::new(adaptor));
    ...
    pub async fn operation(&mut self, op: ChordOperation<I>) -> Option<ChordOperationResult<I>>{
        self.processor_input.send((chord_processor::ProcessorId::Associate(0), op)).await;
        self.processor_output.recv().await
    }

    pub async fn send_op(&mut self, op: ChordOperation<I>){
        self.processor_input.send((chord_processor::ProcessorId::Associate(0), op)).await;
    }

    pub async fn recv_op(&mut self) -> Option<ChordOperationResult<I>>{
        self.processor_output.recv().await
    }
